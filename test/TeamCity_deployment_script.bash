...
# find debian package name to display it in TC job status line
ART_DIR="%teamcity.build.checkoutDir%/debian/"
DEB_PACKAGE=$(find $ART_DIR -name  *.deb -type f | grep -o contentrepo.*\.deb)
echo $DEB_PACKAGE

wraptly repo add -r trusty_$TARGET_ENV -p ubuntu debian/contentrepo_*.deb || exit 1
