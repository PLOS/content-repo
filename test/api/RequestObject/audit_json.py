...
class AuditJson(BaseServiceTest):
  ...
  def get_audit_offset_limit(self,offset, limit):
    ...
    :param limit: limit on the amout of records to return
  ...
  def get_audit_offset_only(self,offset):
    ...
    :param offset: offset to be used to get audit records
    :return:JSON response
    Example:SELECT * FROM audit a ORDER BY a.timestamp LIMIT 1000 OFFSET 0
    """
  ...
  def get_audit_limit_only(self,limit):
    ...
    :param limit: limit on the amout of records to return
    :return:JSON response
    Example: SELECT * FROM audit a ORDER BY a.timestamp LIMIT 200 OFFSET 0
    """
  ...
  def get_audit_offset_limit_sql_bucket (self,offset, limit):
    expected_results_bucket = (MySQL().query("SELECT bucketName FROM audit a ORDER By a.id ASC LIMIT " + str(limit) + " OFFSET " + str(offset)))
    return expected_results_bucket

  def get_audit_offset_limit_sql_key_value (self,offset, limit):
    expected_results_key_value = (MySQL().query("SELECT keyValue FROM audit a ORDER By a.id ASC LIMIT " + str(limit) + " OFFSET " + str(offset)))
    return expected_results_key_value

  def get_audit_offset_limit_sql_operation (self,offset, limit):
    expected_results_operation = (MySQL().query("SELECT operation FROM audit a ORDER By a.id ASC LIMIT " + str(limit) + " OFFSET " + str(offset)))
    return expected_results_operation

  def get_audit_offset_limit_sql_uuid (self,offset, limit):
    expected_results_uuid = (MySQL().query("SELECT uuid FROM audit a ORDER By a.id ASC  LIMIT " + str(limit) + " OFFSET " + str(offset)))
    return expected_results_uuid

  def get_audit_offset_limit_sql_timestamp (self,offset, limit):
    expected_results_timestamp = (MySQL().query("SELECT timestamp FROM audit a ORDER By a.id ASC LIMIT " + str(limit) + " OFFSET " + str(offset)))
  ...
  def verify_audit_text(self, expected_results_full_text, attribute):
    ...
    for actual_full_text_article,expected_full_text_article in zip(actual_source,array_expected_result):
      assert actual_full_text_article[attribute] == expected_full_text_article, (
      '%s is not correct! actual: %s expected: %s' % (attribute, actual_full_text_article[attribute], expected_full_text_article))
