...
class TestCollections(CollectionsJson):
    ...
    def test_get_collections(self):
      ...
      self.verify_http_code_is(OK)
      collections = self.parsed.get_collections()
      assert(len(collections) <= 1000), 'Collection list returned (%d) is greater than default list return set (%d) size or zero' % (str(len(collections)), 1000)
      limit = '%d' % random.randint(1, 1000)
      self.get_collections(bucketName, limit=limit)
      self.verify_http_code_is(OK)
      collections = self.parsed.get_collections()
      assert(str(len(collections)) <= str(limit)), 'Collection list returned (%d) is greater than limit (%d) or zero' % (str(len(collections)), str(limit))
      print('\nDone\n')
